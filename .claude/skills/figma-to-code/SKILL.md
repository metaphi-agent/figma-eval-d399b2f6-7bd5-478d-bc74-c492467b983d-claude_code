...
| `RESUME:building` | Skip to Step 5 |
...
```python
# 1. Get file structure, ask user which page if not specified
...
Install the latest stable version of Tailwind CSS with its Vite plugin:
```bash
...
**src/index.css** (CSS-based theming, no config files):
```css
@import "tailwindcss";
...
- On EVERY user iteration, push changes to the repo BEFORE responding
